...
if($userSick==="0")
{
	header("location:request_error.php?error=7");
...
$semesterStart= "2019-09-01";

$conn=mysqli_connect('localhost','root','','comptech_timeattendance_sahs');
if($conn)
{
	if ($result = mysqli_query($conn, "SELECT COUNT(day_type) AS countCurrentDays FROM anm_daylist WHERE dt BETWEEN '$semesterStart' AND '$today'")) {
        ...
        <div class="wrapper">
			...
			<div class="row">
                    <div class="col-sm-12">
						...
						<button class="btn btn-success" style="margin-bottom:15px" onclick="window.location='sick-days.php'"> Request sick days </button>
                            <table id="datatable" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    <?php
$query= "SELECT requests.request_id AS requestID, COUNT(DISTINCT date) AS countRequests,SUM(value) AS sumDays,requests.*, sick_reason.*, users.* 
FROM requests
LEFT JOIN attendance_sick ON requests.request_id=attendance_sick.request_id
LEFT JOIN sick_reason ON requests.request_id=sick_reason.sick_id
LEFT JOIN users ON requests.staff_id=users.id
WHERE requests.request_type='1' && users.id='$userID'
								...
								if($conn)
{
   ...
   while($info = mysqli_fetch_array( $result ))  {
$countRequests= $info['countRequests'];
$attendance_sickID= $info['id'];
$sApproved= $info['status'];
$sickReason= $info['reason'];
$staffFirstName= $info['firstName'];
$staffLastName= $info['lastName'];
$requestID= $info['requestID'];
$sumDays= $info['sumDays'];
...
{
	$newCountRequests=$countRequests;
}

if($sApproved==="0")
{
	$statusIcon="<button class='btn btn-danger' onclick=window.location='php-update-sick-status.php?ns=1&sid=$requestID'><i class='fa fa-times'></i></button>";
}
if($sApproved==="1")
{
	$statusIcon="<button class='btn btn-primary' onclick=window.location='php-update-sick-status.php?ns=0&sid=$requestID'><i class='fa fa-check'></i></button>";
}


$viewIcon= "<button class='btn btn-success' onclick=window.location='view-sick-days-details.php?id=$requestID'><i class='fa fa-eye'></i></button>";
