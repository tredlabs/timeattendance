                    ...
                    <div class="col-sm-12">
                        <div class="card-box table-responsive">
                            <table id="datatable" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    <?php
$query= "SELECT attendance_staff.staff_id AS cStaffID,attendance_staff.*,schedule_time.*,users.*
FROM attendance_staff
LEFT JOIN schedule_time ON attendance_staff.staff_id=schedule_time.staff_id
LEFT JOIN users ON attendance_staff.staff_id=users.id
WHERE schedule_time.staff_id='$userID' 
";
								if($conn)
{
	if ($result = mysqli_query($conn, $query)) {
   while($info = mysqli_fetch_array( $result ))  {
$cStaffID= $info['cStaffID'];
$cFirstname= $info['firstName'];
$cLastname= $info['lastName'];


$sundayTime= $info['sun'];
$mondayTime= $info['mon'];
$tuesdayTime= $info['tue'];
$wednesdayTime= $info['wed'];
$thursdayTime= $info['thu'];
$fridayTime= $info['fri'];
$saturdayTime= $info['sat'];
