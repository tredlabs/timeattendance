...
$countAttendance= $info['countAttendance'];
	...
	if ($result = mysqli_query($conn, "SELECT COUNT(*) AS countEarly FROM attendance WHERE user_id='$staffID' && status='E'")) {
   while($info = mysqli_fetch_array( $result ))  { 
$countEarly= $info['countEarly'];
		}
	}
	
		if ($result = mysqli_query($conn, "SELECT COUNT(*) AS countLate FROM attendance WHERE user_id='$staffID' && status='L'")) {
   while($info = mysqli_fetch_array( $result ))  { 
								...
								if($conn)
{
	...
	ORDER BY attendance.date ASC, attendance.time DESC")) {
   while($info = mysqli_fetch_array( $result ))  {
