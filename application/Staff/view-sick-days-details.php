...
?>
							...
							<?php
								if($conn)
{
	if ($result = mysqli_query($conn, "SELECT * FROM sick_reason WHERE sick_id='$sickID'")) {
   while($info = mysqli_fetch_array( $result ))  {
$sickReason= $info['reason'];
$sickLeavePhoto= $info['sick_leave'];
echo "<hr/>";
echo "<b>Reason:</b> ".$sickReason;
		}
	}
}
?>
<hr/>
<b>Sick leave image:</b> 
<br/>
<br/>
<!--<a href="../Staff/sick_leave/<?php echo $sickLeavePhoto?>" download>--><img src="../Staff/sick_leave/<?php echo $sickLeavePhoto?>" width="50%" height="50%" alt="No Sick leave image uploaded"/><!--</a>-->
                        </div>
                    </div>
                </div>
