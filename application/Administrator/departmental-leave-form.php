...
if ($result = mysqli_query($conn, "SELECT MAX(request_id) AS maxRequest FROM requests WHERE staff_id='$selectUser' && request_type='2' && status='0'")) {
   while($info = mysqli_fetch_array( $result ))  {
...
if ($result = mysqli_query($conn, "SELECT SUM(value) AS countSickDaysFirst FROM requests WHERE request_type='2' && staff_id='$selectUser' && status='1'")) {
while($info = mysqli_fetch_array( $result ))  { 
...
if ($result = mysqli_query($conn, "SELECT MAX(date) AS maxRequestDate, MIN(date) AS minRequestDate FROM requests WHERE staff_id='$selectUser' && request_type='2' && request_id='$maxRequest'")) {
while($info = mysqli_fetch_array( $result ))  { 
			...
			<div class="row">
                    <div class="col-lg-12">
                        <div class="card-box">
							...
							<br/>
							<br/>
							<p>I <b><?php echo $selectUserFirstname. " " .$selectUserLastname?></b> hereby apply for <b><?php echo $newCountNoDays?></b> days. departmental leave from <b><?php echo $minRequestDate1?></b> to <b><?php echo $maxRequestDate1?></b> inclusive.
							</p>
