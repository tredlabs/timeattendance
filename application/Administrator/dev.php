...
$staffID= base64_decode($_REQUEST['id']);



	if ($result = mysqli_query($conn, "SELECT COUNT(*) AS countAttendance FROM attendance WHERE user_id='$staffID'")) {
   while($info = mysqli_fetch_array( $result ))  { 
$countAttendance= $info['countAttendance'];
		}
	}
	...
	if ($result = mysqli_query($conn, "SELECT COUNT(DISTINCT date) AS countEarly FROM attendance WHERE user_id='$staffID' && status='E'")) {
   while($info = mysqli_fetch_array( $result ))  { 
$countEarly= $info['countEarly'];
		}
	}
	
	
		if ($result = mysqli_query($conn, "SELECT COUNT(DISTINCT date) AS countLate FROM attendance WHERE user_id='$staffID' && status='L'")) {
   while($info = mysqli_fetch_array( $result ))  { 
$countLate= $info['countLate'];
		}
	}


	
	$percentEarly= round(($countEarly/$countAttendance)*100);
	
	$percentLate= round(($countLate/$countAttendance)*100);
				...
				<div class="row">
                    <div class="col-sm-12">
                        ...
                        <div class="card-box table-responsive">
							...
							<!--Average early clock in: <?php echo $percentEarly?>% |	Average late clock in: <?php echo $percentLate?>%
					<hr/>-->
                                <thead>
                                    <tr>
                                        <th>No.</th>
								...
								if($conn)
{
	if ($result = mysqli_query($conn, "SELECT attendance.*,attendance_staff.*,users.*
	...
	ORDER BY attendance.date ASC,attendance.activity ASC")) {
   while($info = mysqli_fetch_array( $result ))  {
$staffFirstName= $info['firstName'];
$staffLastName= $info['lastName'];
$staffID= $info['staff_id'];
$attendanceStatus= $info['status'];
$attendanceTime= date_create($info['time']);
$attendanceTime1= date_format($attendanceTime, "g:i a");


$attendanceDate= date_create($info['date']);
$attendanceDate1= date_format($attendanceDate, "F d, Y");
...
if($attendanceStatus==="E")
	...
	$x++;
                                    ...
                                    <td>$staffLastName</td>
                                    <td>$aFull</td>
                                    <td>$attendanceDate1</td>
                                    <td>$attendanceTime1</td>
                                    <td style='background-color:$background; color:$text;'>$newStatus</td>
                                    </tr>
									";
		}
	}
