...
$daysPassed= $info['countCurrentDays'];
		...
		if ($result = mysqli_query($conn, "SELECT * FROM attendance_limit WHERE id='1'")) {
   while($info = mysqli_fetch_array( $result ))  { 
$minLimit= $info['min'];
		}
	}
				...
				if($conn)
{
   ...
   while($info = mysqli_fetch_array( $result ))  {
$daysAttended= $info['countAttended'];
$noDays= $info['no_days'];
$staffFirstName= $info['firstName'];
$staffLastName= $info['lastName'];
$staffID= base64_encode($info['newStaff_id']);
$staffID1= $info['staff_id'];
$user_id= $info['user_id'];
$countSick= $info['countSick'];
$countVacation= $info['countVacation'];
$countCasual= $info['countCasual'];
//$countLeave= $info['countLeave'];
$countSpecial= $info['countSpecial'];
$countDays= $info['date'];
$attendanceStatus= $info['status'];
$casualDays= $info['casual'];
$vacationDays= $info['vacation'];
$specialDays= $info['special'];
$sickDays= $info['sick'];
$testDaysPassed= $info['passed'];





$days_passed= $info['passed'];
$dateEnrolled= $info['enroll_date'];




//$daysPassedForEach= $daysPassed-$days_passed;
...
#$staffAPercentage= ceil(($fullDaysAttended/$testDaysPassed)*100);
//$staffAPercentage= round(($daysAttended/$testDaysPassed)*100);

//echo"
//<!--Change $test days passed to this below here.
//$daysPassedForEach-->
//";
...
{
	$newCountSick=$countSick;
}



if($countVacation===NULL)
{
	$newCountVacation=0;
}
else
{
	$newCountVacation=$countVacation;
}
...
{
	$newCountSpecial=$countSpecial;
}

//This is for the attendance percentage
//$newAttendancePercentage=round(($countCasual+$daysAttended)/$testDaysPassed*100);
//REPLACE WITH$newAttendancePercentage=round(($countCasual+$daysAttended)/$noDays*100);
//End

/*
if($newAttendancePercentage>100)
{
	$fullAttendancePercentage=100;
}
else
{
	$fullAttendancePercentage=$newAttendancePercentage;
}*/
...
if($newAttendancePercentage<$minLimit)
{
	$staffStatus= "<button class='btn btn-icon waves-effect btn-danger m-b-5'> <i class='fa fa-level-down'></i></button>";
}
else
{
	$staffStatus= "<button class='btn btn-icon waves-effect btn-primary m-b-5'> <i class='fa fa-level-up'></i></button>";
}
*/

//$staffAPercentage $fullAttendancePercentage  $testDaysPassed<!--PUT THIS BACK$daysPassedForEach--><!--$noDays-->  $staffStatus
$viewIcon= "<button class='btn btn-success' onclick=window.location='view-staff-attendance-details.php?id=$user_id';cal($user_id);><i class='fa fa-eye'></i></button>";
			echo"
			<tr>
			...
			<td>$daysAttended</td>
			<td></td>
			<td>$newCountSick/$sickDays</td>
			<td>$newCountVacation/$vacationDays</td>
			<td>$newCountCasual/$casualDays</td>
			<td>$newCountSpecial/$specialDays</td>
			<td></td>
			<td>$viewIcon</td>
			</tr>
			";
